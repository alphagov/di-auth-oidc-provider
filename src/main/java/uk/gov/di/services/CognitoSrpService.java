...
public class CognitoSrpService extends CognitoService {
    ...
    public boolean performSRPAuthentication(String username, String password) {
        ...
        var a = srp6Routines.generatePrivateValue(N, new SecureRandom());
        THREAD_MESSAGE_DIGEST.get().reset();

        var A = srp6Routines.computePublicClientValue(N, g, a);
    ...
    private RespondToAuthChallengeRequest userSrpAuthRequest(SRP6CryptoParams config,
        ...
        String usernameInternal = initiateAuthResponse.challengeParameters().get("USERNAME");
        ...
        byte[] key = getPasswordAuthenticationKey(config, userIdForSRP, password, B, salt, A, a);
    ...
    private byte[] getPasswordAuthenticationKey(SRP6CryptoParams config,
                                                String userId,
                                                String userPassword,
                                                BigInteger B,
                                                BigInteger salt,
        ...
        byte[] key = hkdf.deriveKey("Key Info", 16);
        return key;
    }
    ...
    final static class Hkdf {
        ...
        private byte[] deriveKey(String info, int length) {
            return this.deriveKey(info != null ? info.getBytes() : null, length);
        }
